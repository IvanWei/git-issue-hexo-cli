...
const {writeFile, removeFile, namedFile, formatPostDate} = require('../utils');
...
function refresh (username, repoName, gitToken, label, outputPath) {
  ...
  .then(async (res) => {
    ...
    const newFiles = result.filter((issue) => {
      ...
      const labels = issue.labels.filter((label) => (label.name !== PUBLISHED_LABEL))
      ...
      return writeFile(newFilename, content, outputPath);
    });
